...
class CommandLibraryDialog(QDialog):
    ...
    def agregar_nuevo_parametro(
        ...
        label_veces.setAlignment(
            Qt.AlignCenter
        )

        repeticiones = QSpinBox()

        repeticiones.setRange(
            1,
            99,
        )
        ...
        repeticiones.setToolTip(
            "Ejemplo: Tab elevado a 3 = pulsar Tab tres veces."
        )
        ...
        parametro.addItems(
            self.opciones_teclado()
        )

        parametro.setCurrentText(
            "Tab"
        )
        ...
        boton_eliminar.clicked.connect(
            lambda checked=False, item=registro:
            self.eliminar_parametro(item)
        )
        ...
        if self.parametros_widgets:
            ...
            self.parametros_layout.addWidget(
                simbolo
            )
        ...
        self.parametros_widgets.append(
            registro
        )
    ...
    def eliminar_parametro(
        ...
        if len(self.parametros_widgets) <= 1:
            QMessageBox.information(
                self,
                "Parámetro requerido",
                "Un comando debe conservar al menos un parámetro.",
            )

            return

        if registro not in self.parametros_widgets:
            return

        self.parametros_widgets.remove(
            registro
        )

        simbolo = registro.get(
            "simbolo"
        )
        ...
        registro["marco"].deleteLater()

        self.reconstruir_parametros()

    # ========================================================
    # RECONSTRUIR FILA
    # ========================================================

    def reconstruir_parametros(
        self,
    ):
        while self.parametros_layout.count():
            ...
            if widget is not None:
                self.parametros_layout.removeWidget(
                    widget
                )
    ...
    def obtener_parametros(
        ...
        for registro in self.parametros_widgets:
            tecla = registro[
                "parametro"
            ].currentText().strip()
    ...
    # CREAR COMANDO
    # ========================================================
    def cargar_parametros_en_constructor(
        self,
        parametros,
    ):
        while self.parametros_layout.count():
            item = self.parametros_layout.takeAt(
                0
            )
    ...
    def eliminar_comando_guardado(
        ...
        respuesta = QMessageBox.warning(
            self,
            "Eliminar comando",
            f"Vas a eliminar '{comando.get('nombre', 'Comando')}'.\n\n"
            f"{afectadas} tarea(s) guardada(s) utilizan "
            "este comando.\n\n"
            "La acción asociada a este comando será "
            "ELIMINADA de todas esas tareas.\n\n"
        ...
        if (
            ...
            and self.comando_seleccionado.get(
                "id"
            )
            == comando_id
        ):
        ...
        if (
            ...
            and hasattr(
                padre,
                "eliminar_comando_de_tareas",
            )
        ):
            padre.eliminar_comando_de_tareas(
                comando_id
            )
        ...
        if (
            ...
            and hasattr(
                padre,
                "guardar_biblioteca_comandos",
            )
        ):
            padre.guardar_biblioteca_comandos(
                self.biblioteca
            )
    ...
    def crear_comando(
        ...
        if not nombre:
            QMessageBox.warning(
                self,
                "Nombre requerido",
                "Escribe un nombre para el comando.",
            )
        ...
        if not parametros:
            QMessageBox.warning(
                ...
                "Añade al menos un parámetro de teclado.",
            )
        ...
        if self.comando_editando_id:
            ...
            if (
                ...
                and hasattr(
                    padre,
                    "actualizar_comando_en_tareas",
                )
            ):
                padre.actualizar_comando_en_tareas(
                    comando
                )
    ...
    def refrescar_lista_comandos(
        self,
    ):
        while self.layout_comandos.count():
            item = self.layout_comandos.takeAt(0)
        ...
        for comando in self.biblioteca:
            ...
            boton.clicked.connect(
                ...
                identificador=comando_id:
                self.seleccionar_comando(
                    identificador
                )
            )
            ...
            boton_eliminar.clicked.connect(
                lambda checked=False,
                identificador=comando_id:
                self.eliminar_comando_guardado(
                    identificador
                )
            )
            ...
            self.layout_comandos.addWidget(
                fila
            )
    ...
    def iniciar_captura_coordenadas(
        self,
    ):
        if pynput_mouse is None:
            QMessageBox.warning(
                ...
                "BIN necesita pynput para capturar "
                "la posición del mouse.",
            )

            return

        if self.listener_captura_coordenadas is not None:
            try:
                self.listener_captura_coordenadas.stop()
            except Exception:
                pass

            self.listener_captura_coordenadas = None

        self.boton_simular_click.setText(
        ...
        def capturar(
            ...
            # La captura de coordenadas se confirma
            # siempre con clic izquierdo.
            if boton != pynput_mouse.Button.left:
                return

            self.coordenadas_capturadas.emit(
                int(x),
                int(y),
            )
        ...
        try:
            self.listener_captura_coordenadas = (
            ...
            self.listener_captura_coordenadas.start()

        except Exception as error:
            self.recuperar_dialogo_despues_captura()

            QMessageBox.warning(
                self,
                "Captura de coordenadas",
                "No pude iniciar la captura.\n\n"
                f"{error}",
            )

    # ========================================================
    # RECIBIR COORDENADAS CAPTURADAS
    # ========================================================

    def recibir_coordenadas_capturadas(
        self,
        x,
        y,
    ):
    ...
    def recuperar_dialogo_despues_captura(
        self,
    ):
        padre = self.parent()
    ...
    def agregar_accion(self):
        ...
        mantener_original = (
            ...
            and self.usar_accion_original
            and self.accion_editar is not None
        )
...
class TaskCard(QFrame):
    ...
    def crear_interfaz(self):
        ...
        self.boton_contexto = QPushButton(
            "CONTEXTO ▸"
        )

        self.boton_contexto.setObjectName(
            "botonContexto"
        )

        self.boton_contexto.setCheckable(
            True
        )

        contexto_actual = self.tarea.get(
            "contexto",
            "",
        )

        self.boton_contexto.setToolTip(
            contexto_actual
            if contexto_actual
            else "Sin contexto guardado"
        )
        ...
        self.contenedor_contexto = QFrame()

        self.contenedor_contexto.setObjectName(
            "panel"
        )

        contexto_layout = QVBoxLayout(
            self.contenedor_contexto
        )

        contexto_layout.setContentsMargins(
            8,
            8,
            8,
            8,
        ...
        contexto_layout.setSpacing(
            6
        )

        self.editor_contexto = QTextEdit()

        self.editor_contexto.setPlaceholderText(
            "Explica qué hace esta tarea automatizada..."
        )

        self.editor_contexto.setPlainText(
            contexto_actual
        )

        self.editor_contexto.setReadOnly(
            True
        )
        ...
        contexto_layout.addWidget(
            self.editor_contexto
        )

        botones_contexto = QHBoxLayout()

        self.boton_editar_contexto = QPushButton(
            "EDITAR"
        )

        self.boton_guardar_contexto = QPushButton(
            "GUARDAR CAMBIOS"
        )

        self.boton_guardar_contexto.setEnabled(
            False
        )

        self.boton_editar_contexto.clicked.connect(
            self.activar_edicion_contexto
        )

        self.boton_guardar_contexto.clicked.connect(
            self.guardar_contexto_local
        )

        botones_contexto.addWidget(
            self.boton_editar_contexto
        )

        botones_contexto.addWidget(
            self.boton_guardar_contexto
        )

        contexto_layout.addLayout(
            botones_contexto
        )

        self.contenedor_contexto.hide()

        self.boton_contexto.toggled.connect(
            self.alternar_contexto
        )
        ...
        self.boton_reset = QPushButton("RESETEAR TAREA")

        self.boton_reset.setObjectName("botonReset")
    ...
    def alternar_contexto(
        self,
        abierto,
    ):
        self.contenedor_contexto.setVisible(
            abierto
        )

        if abierto:
            self.boton_contexto.setText(
                "CONTEXTO ▾"
            )
        else:
            self.boton_contexto.setText(
    ...
    def activar_edicion_contexto(
        ...
        self.editor_contexto.setFocus()

        self.boton_editar_contexto.setEnabled(
            False
        )
...
class BIN(QMainWindow):
    ...
    def __init__(self):
        ...
        # ====================================================
        # SUPERFICIES TRANSITORIAS DE WINDOWS
        # ====================================================

        self.menu_inicio_replay_hasta = 0.0

        self.modo_shell_windows = False
        ...
        self.boton_detener_ejecucion = None
        ...
        if self.ruta_icono_bin.exists():

            self.icono_bin = QIcon(
                str(
                    self.ruta_icono_bin
                )
            )

            if not self.icono_bin.isNull():
    ...
    def crear_interfaz(self):
        ...
        identidad_widget = QWidget()

        identidad_widget.setObjectName(
            "identidadBin"
        )
        ...
        self.cara_bin = QLabel()

        self.cara_bin.setObjectName(
            "caraBin"
        )

        self.cara_bin.setFixedSize(
            52,
            52,
        )

        self.cara_bin.setAlignment(
            Qt.AlignCenter
        )
        ...
        if ruta_cara_bin.exists():

            icono_cara_bin = QIcon(
                str(
                    ruta_cara_bin
                )
            )

            if not icono_cara_bin.isNull():

                self.cara_bin.setPixmap(
                    icono_cara_bin.pixmap(
                        52,
                        52,
                    )
                )
        ...
        textos_identidad = QVBoxLayout()

        textos_identidad.setContentsMargins(
            0,
            0,
            0,
            0,
        ...
        entrada_chat_layout.setContentsMargins(
            0,
            0,
            0,
            0,
        ...
        self.boton_limpiar_panel = QPushButton("LIMPIAR PANEL DE ACCIÓN")

        self.boton_limpiar_panel.clicked.connect(
            lambda: self.limpiar_panel_accion()
        )

        acciones_layout.addWidget(self.boton_limpiar_panel)
    ...
    def limpiar_panel_accion(
        ...
        if self.grabando:
            if not silencioso:
                QMessageBox.information(
                    self,
                    "Grabación activa",
                    "Detén primero la grabación antes de limpiar "
                    "el Panel de acción.",
                )
        ...
        if not silencioso:
            self.actualizar_chat_bin(
                "Limpié el Panel de acción. "
                "Las secuencias guardadas en las tareas permanecen intactas."
            )
    ...
    def resetear_tarea(
        ...
        if tarea.get("estado") in [
            ...
            QMessageBox.information(
                self,
                "Tarea activa",
                "Detén primero la ejecución antes de resetear la tarea.",
            )
        ...
        respuesta = QMessageBox.question(
            self,
            "Resetear tarea",
            "BIN reiniciará únicamente el estado de ejecución.\n\n"
            "La secuencia, las acciones, el horario y la configuración "
            "de la tarea NO serán eliminados.\n\n"
            f"¿Resetear '{tarea.get('nombre', 'Tarea')}'?",
            QMessageBox.Yes | QMessageBox.No,
        )
        ...
        run_key_anterior = tarea.get("current_run_key")

        if run_key_anterior:
            tarea["last_skipped_run_key"] = run_key_anterior
        ...
        if tarea.get("acciones"):
            tarea["detalle_estado"] = (
                "Tarea reseteada · Lista para iniciar desde cero"
            )
        else:
            tarea["detalle_estado"] = (
                "Tarea reseteada · Sin acciones configuradas"
            )

        self.guardar_tareas_en_disco()

        self.actualizar_tarjeta_tarea(tarea_id)

        self.refrescar_lista_tareas()

        self.actualizar_chat_bin(
            f"Reseteé '{tarea['nombre']}'. "
            "Su secuencia permanece guardada."
        )

    # ========================================================
    # CREAR BARRA FLOTANTE DE EJECUCIÓN
    # ========================================================
    ...
    def crear_barra_ejecucion(
        ...
        self.label_barra_ejecucion = QLabel(
            "BIN · Preparando ejecución..."
        )

        self.boton_detener_ejecucion = QPushButton(
            "DETENER ACCIÓN"
        )

        self.boton_detener_ejecucion.clicked.connect(
            self.detener_ejecucion_desde_barra
        )

        layout.addWidget(
            self.label_barra_ejecucion,
            1,
        )

        layout.addWidget(
            self.boton_detener_ejecucion,
        )
    ...
    def actualizar_barra_ejecucion(
        self,
        tarea=None,
    ):
        ...
        plan = self.plan_ejecucion_actual or self.obtener_plan_ejecucion(tarea)

        cantidad_acciones = max(
            1,
            len(plan),
        )
        ...
        repeticion = max(
            ...
            int(
                ...
                or tarea.get("repeticion_actual", 1)
            ),
        )
        ...
        self.label_barra_ejecucion.setText(
            f"{nombre}   ·   "
            f"Repetición {repeticion}/{repeticiones}   ·   "
            f"Paso {paso}/{cantidad_acciones}"
        )
    ...
    def mostrar_barra_ejecucion(
        ...
        self.actualizar_barra_ejecucion(tarea)
    ...
    # ========================================================
    # DETENER EJECUCIÓN DESDE BARRA
    # ========================================================

    def detener_ejecucion_desde_barra(
        self,
    ):
        tarea = self.obtener_tarea_ejecutando()
        ...
        def comando(
            ...
            return {
                ...
                "parametros": [
                    ...
                    for tecla, repeticiones in parametros
                ],
                "personalizado": False,
            }
    ...
    def cargar_biblioteca_comandos(
        self,
    ):
        ruta = (
            self.ruta_biblioteca_comandos()
        )
        ...
        return self.comandos_teclado_base()
    def guardar_biblioteca_comandos(
        self,
        biblioteca,
    ):
        ruta = self.ruta_biblioteca_comandos()
        ...
        try:
            ...
            QMessageBox.warning(
                self,
                "Biblioteca de comandos",
                "No pude guardar la biblioteca.\n\n"
                f"{error}",
            )

    def texto_parametros_comando(
        self,
        parametros,
    ):
        partes = []

        for parametro in parametros or []:
            tecla = str(
                parametro.get(
                    "tecla",
                    "",
    ...
    def descripcion_accion_comando(
        self,
        nombre,
        parametros,
        espera_antes_ms=0,
        ...
        texto = (
            self.texto_parametros_comando(
                parametros
            )
        )

        descripcion = (
            f"Comando · {nombre} · {texto}"
        )

        if espera_antes_ms > 0:
            descripcion += (
                f" · antes "
                f"{espera_antes_ms / 1000:g}s"
            )

        if espera_despues_ms > 0:
            descripcion += (
                f" · después "
                f"{espera_despues_ms / 1000:g}s"
            )
    ...
    def agregar_accion_manual(
        ...
        if acciones is None:
            QMessageBox.information(
                ...
                "Primero crea o selecciona una tarea.",
            )
        ...
        # Los comandos personalizados creados dentro del
        # diálogo quedan almacenados globalmente.
        self.guardar_biblioteca_comandos(
            dialogo.biblioteca
        )
        ...
        accion = dialogo.accion_resultado
    ...
    def editar_accion_manual(
        ...
        dialogo = CommandLibraryDialog(
            self,
            biblioteca=biblioteca,
            accion_editar=accion_actual,
        )

        resultado = dialogo.exec()

        # Cualquier comando nuevo creado durante la edición
        # queda almacenado en la biblioteca global.
        self.guardar_biblioteca_comandos(
            dialogo.biblioteca
        )
        ...
        nueva_accion = dialogo.accion_resultado

        if not nueva_accion:
            return

        # ----------------------------------------------------
        # CONSERVAR POSICIÓN E ID
        # ----------------------------------------------------

        id_original = accion_actual.get(
            "id"
        )

        nueva_accion["id"] = id_original

        acciones[indice] = nueva_accion
    ...
    def estimar_duracion_comando_ms(
        self,
        accion,
    ):
        datos = accion.get(
        ...
        espera_antes = accion.get(
            "espera_antes_ms"
        )

        if espera_antes is None:
            espera_antes = datos.get(
                "espera_antes_ms",
                0,
            )

        espera_despues = accion.get(
            "espera_despues_ms"
        )

        if espera_despues is None:
            espera_despues = datos.get(
                "espera_despues_ms",
                0,
            )
    ...
    def recalcular_duracion_desde_acciones(
        self,
        tarea,
    ):
        origen = tarea.get(
        ...
        # Las demostraciones reales ya poseen una duración
        # tomada durante la grabación.
        if origen in [
            "grabacion",
            "real",
            "mixta_grabacion",
        ...
        for accion in acciones:
            ...
            if tipo == "comando_teclado":
                total_ms += self.estimar_duracion_comando_ms(
                    accion
                )
    ...
    def guardar_rutina_como_tarea(self):
        ...
        self.rutina_borrador_semantica = []
        self.rutina_en_borrador = False
        self.grabacion_destino_tarea_id = None

        self.cantidad_acciones_base_grabacion = 0

        self.modo_grabacion_actual = "nueva"

        self.duracion_base_grabacion_segundos = 0
        self.duracion_rutina_borrador = 0
        self.inicio_grabacion = None
        self.tarea_seleccionada_id = nuevo_id
    ...
    def obtener_plan_ejecucion(
        ...
        """
        BIN Light ejecuta directamente la secuencia real
        almacenada en el Panel de acciones.

        Esto permite mezclar en una misma tarea:
        ...
        - teclas
        - atajos grabados
        - comandos de teclado creados manualmente

        acciones_semanticas se conserva como información
        auxiliar, pero NO reemplaza la secuencia física.
        """
    ...
    def calcular_offsets_plan_ejecucion(
        ...
        duracion_ms = max(
            1,
            self.obtener_duracion_repeticion(
                tarea
            )
            * 1000,
        )
        ...
        if not marcas_validas:
            ...
            for accion in plan:
                ...
                if (
                    ...
                    acumulado_ms += (
                        self.estimar_duracion_comando_ms(
                            accion
                        )
                    )
        ...
        # ====================================================
        # RUTINA GRABADA O MIXTA
        # ====================================================

        inicio_grabacion = min(
            marcas_validas
        )
        ...
        tiempo_manual_acumulado = 0

        for accion, marca in zip(
            plan,
            marcas,
        ):
            if marca is not None:
    ...
    def ejecutar_windows_solo(
        self,
    ):
        """
        Ejecuta la tecla Windows como una tecla operativa
        independiente y no como un simple modificador.

        Win + R, Win + E, etc. siguen usando el motor normal.
        """
        ...
        if sys.platform != "win32":
            ...
            try:
                tecla_win = pynput_keyboard.Key.cmd
        ...
        try:
            ...
            self.activar_modo_shell_windows()

            return True

        except Exception as error:
            self.actualizar_chat_bin(
                "No pude ejecutar la tecla Windows.\n\n"
                f"{error}"
            )
    ...
    def ejecutar_atajo_replay(
        ...
        tecla_normalizada = str(
            tecla or ""
        ).strip().lower()

        if (
            not modificadores_normalizados
            and tecla_normalizada
            in {
                "win",
                "windows",
                "cmd",
            ...
            return self.ejecutar_windows_solo()
    ...
    # ========================================================
    # EJECUTAR COMANDO DE TECLADO PERSONALIZADO
    # ========================================================

    def tecla_comando_a_replay(
        self,
        nombre,
    ):
        texto = str(
        ...
        if len(texto) == 1:
            ...
            return self.tecla_replay_desde_nombre(
                caracter,
                caracter,
            )
    ...
    def ejecutar_comando_teclado(
        ...
        if len(
            ...
            if (
                ...
                and repeticiones_unicas == 1
            ):
                ok = self.ejecutar_windows_solo()
        ...
        if not self.asegurar_controladores_replay():
            return {
                ...
                "metodo": "pynput",
                "detalle": (
                    "Controlador de teclado no disponible"
                ),
            }

        modificadores_presionados = []
        ...
        try:
            for parametro in parametros or []:
                ...
                modificador = self.modificador_replay(
                    nombre
                )

                # --------------------------------------------
                # MODIFICADORES:
                # se mantienen pulsados hasta terminar
                # todo el comando.
                # --------------------------------------------
                ...
                if modificador is not None:
                    if (
                        modificador
                        not in modificadores_presionados
                    ):
                        self.keyboard_replay.press(
                            modificador
                        )
                ...
                tecla = self.tecla_comando_a_replay(
                    nombre
                )
    ...
    # ========================================================
    # MODO ESPECIAL PARA MENÚ INICIO / SHELL DE WINDOWS
    # ========================================================

    def activar_modo_shell_windows(
        self,
    ):
        self.modo_shell_windows = True
    ...
    def desactivar_modo_shell_windows(
        self,
    ):
        self.modo_shell_windows = False
    ...
    def contexto_es_superficie_transitoria_windows(
        self,
        contexto,
    ):
        if not contexto:
        ...
        if proceso in procesos_transitorios:
            return True
        ...
        return any(
            indicador in texto_contexto
            for indicador in indicadores
        )

    def accion_requiere_contexto_activo(
        self,
        accion,
    ):
        if not accion:
        ...
        # ----------------------------------------------------
        # ACCIONES QUE CAMBIAN EL FOREGROUND POR SÍ MISMAS
        # ----------------------------------------------------
        ...
        if tipo in tipos_mouse:
            ...
            # =================================================
            # MENÚ INICIO / SEARCH / SHELL
            # =================================================
            #
            # Son ventanas transitorias.
            #
            # NO intentamos activarlas mediante SetForegroundWindow.
            # El clic debe ir directamente a las coordenadas
            # que están visibles.
            # =================================================

            if self.replay_menu_inicio_activo():
                return False
    ...
    def ejecutar_accion_original_real(
        ...
        if tipo in {
            ...
            # =================================================
            # MENÚ INICIO / SEARCH / SHELL DE WINDOWS
            # =================================================

            click_superficie_transitoria = (
                self.replay_menu_inicio_activo()
                or
                self.contexto_es_superficie_transitoria_windows(
                    contexto
                )
            )
            ...
            delay_bin = (
                900
                if click_superficie_transitoria
                else 0
            )
            ...
            if tipo == "click_derecho":
                ...
                for indice_click in range(
                    cantidad_clicks
                ):
                    resultado_click = (
                        self.ejecutar_mouse_fisico(
                            x,
                            y,
                            "click_derecho",

                            restaurar_bin_delay_ms=(
                                0
                                if click_superficie_transitoria
                                else (
                                    delay_bin
            ...
            if click_superficie_transitoria:
                ...
                # Dejamos que StartMenuExperienceHost procese
                # realmente el clic antes de recuperar BIN.
                if sys.platform == "win32":
                    ctypes.windll.kernel32.Sleep(
                        1200
                    )

                self.desactivar_modo_shell_windows()
        ...
        if tipo in {
            ...
            if not self.preparar_contexto_teclado_replay(
                contexto
            ):
                return {
                    "ok": False,
            ...
            if ok:
                ...
                es_win_solo = (
                    ...
                    and not modificadores_normalizados
                )

                if es_win_solo:
                    # Windows necesita un pequeño margen para
                    # terminar de desplegar el menú Inicio.
                    self.menu_inicio_replay_hasta = (
                        time.monotonic()
                        + 10.0
                    )
    ...
    def iniciar_ejecutor_real(
        ...
        # ====================================================
        #
        # La ejecución se realiza siempre desde la secuencia
        # física/original almacenada en tarea["acciones"].
        #
        # De esta manera una misma tarea puede contener:
        #
        #   acción grabada
        #   acción grabada
        #   comando manual
        ...
        #   comando manual
        #
        # sin que acciones_semanticas sustituya la demostración.
        # ====================================================
    ...
    # ========================================================
    # OBTENER ESPERA CONFIGURADA DE UNA ACCIÓN
    # ========================================================

    def obtener_espera_accion_ms(
        self,
        accion,
        campo,
    ):
        ...
        # Las acciones semánticas guardan los datos
        # dentro de "datos".
        if valor is None:
            datos = accion.get(
                "datos"
    ...
    def procesar_timer_ejecucion_real(self):
        ...
        # ====================================================
        # ESPERA ANTES DE UNA ACCIÓN MANUAL
        # ====================================================

        if self.fase_ejecucion_real == "espera_antes_accion":
            self.fase_ejecucion_real = (
                "ejecutar_tras_espera_antes"
            )
        ...
        # ====================================================
        # ESPERA DESPUÉS DE UNA ACCIÓN MANUAL
        # ====================================================

        if self.fase_ejecucion_real == "espera_despues_accion":
            tarea = self.obtener_tarea_ejecutando()
    ...
    def ejecutar_siguiente_accion_real(self):
        ...
        espera_antes_ms = self.obtener_espera_accion_ms(
            accion,
            "espera_antes_ms",
        )
        ...
        if (
            ...
            tarea["detalle_estado"] = (
                "Esperando antes de ejecutar "
                f"la acción {self.indice_ejecucion_real + 1}"
            )
    ...
    def procesar_checkpoint_accion_real(self):
        ...
        if decision == "READY":
            ...
            if espera_despues_ms > 0:
                ...
                tarea["detalle_estado"] = (
                    "Acción completada · "
                    "esperando antes del siguiente paso"
                )
    ...
    def iniciar_ejecucion(
        ...
        self.mostrar_barra_ejecucion(tarea)
    ...
    def procesar_evento_teclado_global(
        ...
        if tipo_evento == "down":
            ...
            # =================================================
            #
            # Los atajos detectados durante una demostración
            # NO se guardan como "atajo_teclado".
            #
            # Se transforman automáticamente al mismo formato
            # de comando que ya sabemos que BIN ejecuta bien.
            # =================================================
            ...
            if es_atajo:
                ...
                # ---------------------------------------------
                # CONSTRUIR LOS PARÁMETROS DEL COMANDO
                #
                # Ejemplo:
                #
                # Ctrl + Shift + 1
                ...
                for modificador_actual in modificadores:
                    ...
                    parametros_comando.append(
                        {
                            "tecla": modificador_actual,
                            "repeticiones": 1,
                        }
                    )
                ...
                accion_id = self.registrar_accion_global(
                    ...
                    # Igual que los comandos creados
                    # manualmente: no vinculamos el comando
                    # a un HWND histórico concreto.
                    None,
    ...
    def alternar_grabacion(
        ...
        if not self.grabando:
            ...
            # =================================================
            # DECIDIR SI COMPLEMENTAMOS O REEMPLAZAMOS
            # =================================================

            self.grabacion_destino_tarea_id = (
                self.tarea_seleccionada_id
            )

            acciones_base = []

            self.modo_grabacion_actual = (
                "nueva"
            )
            ...
            if (
                ...
                tarea_destino = self.obtener_tarea(
                    self.grabacion_destino_tarea_id
                )

                if tarea_destino:
                    if tarea_destino.get(
                        "estado"
                    ) in [
                        "EJECUTANDO",
                        "EN COLA",
                    ...
                    self.duracion_base_grabacion_segundos = (
                        duracion_a_segundos(
                            tarea_destino.get(
                                "duracion",
                                "00:00:00",
                    ...
                    if existentes:
                        ...
                        pregunta.setText(
                            "Esta tarea ya tiene acciones.\n\n"
                            "¿Qué quieres hacer con la nueva demostración?"
                        )

                        boton_complementar = (
                            pregunta.addButton(
                                "COMPLEMENTAR ACCIONES DEL PANEL",
                        ...
                        pulsado = (
                            pregunta.clickedButton()
                        )

                        if pulsado is boton_complementar:
                            acciones_base = json.loads(
                                json.dumps(
                                    existentes,
                                    ensure_ascii=False,
                                )
            ...
            self.rutina_borrador = json.loads(
                json.dumps(
                    acciones_base,
                    ensure_ascii=False,
                )
            ...
            self.cantidad_acciones_base_grabacion = len(
                self.rutina_borrador
            )
        ...
        # ====================================================
        # AGRUPAR SOLAMENTE LO NUEVO
        # ====================================================

        cantidad_base = max(
            0,
            int(
                self.cantidad_acciones_base_grabacion
            ),
        )
        ...
        # ====================================================
        # SI ESTÁBAMOS MODIFICANDO UNA TAREA EXISTENTE,
        # GUARDAR EL RESULTADO DIRECTAMENTE EN ELLA.
        # ====================================================
        ...
        if (
            ...
            if tarea_destino:
                tarea_destino["acciones"] = json.loads(
                    json.dumps(
                        self.rutina_borrador,
                        ensure_ascii=False,
                    )
                ...
                tarea_destino["duracion_origen"] = (
                    "mixta_grabacion"
                    if self.modo_grabacion_actual
                    == "complementar"
                    else "grabacion"
                )

                tarea_destino["detalle_estado"] = (
                    f"{len(tarea_destino['acciones'])} "
    ...
    def ejecutar_mouse_fisico(
        ...
        tipo,
        delta=0,
        restaurar_bin_delay_ms=0,
        restaurar_cursor_delay_ms=0,
        mantener_bin_abajo=False,
        mantener_cursor_en_destino=False,
    ):
        ...
        try:
            ...
            if restaurar_cursor_delay_ms > 0:
            ...
            if (
                posicion_guardada
                and not mantener_cursor_en_destino
            ):
    ...
    def aplicar_estilos(self):
        ...
        self.setStyleSheet(f"""
            ...
            QPushButton#botonContexto:hover {{
