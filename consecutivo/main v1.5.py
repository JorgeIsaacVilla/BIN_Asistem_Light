    ...
    def validar(self):
        ...
        if self.validador_conflictos:
            ...
            if conflictos:
                ...
                if (
                    parent is not None
                    and hasattr(parent, "formatear_mensaje_conflictos")
                ):
...
class BIN(QMainWindow):
    ...
    def __init__(self):
        ...
        self.timer_click_global.timeout.connect(
            self.confirmar_click_global_pendiente
        )

        self.evento_mouse_global.connect(
            self.procesar_evento_mouse_global
        )

        self.evento_teclado_global.connect(
            self.procesar_evento_teclado_global
        )
        ...
        self.solicitud_detener_grabacion.connect(
            self.detener_grabacion_desde_atajo
        )
        ...
        for tarea in self.tareas:
            ...
            if tarea.get("current_run_key"):
                ...
                if estado_repeticion["duracion_repeticion"] > 0:
                    tarea["repeticion_actual"] = estado_repeticion[
                        "repeticion_actual"
                    ]
    ...
    def editar_tarea(self, tarea_id):
        ...
        repeticiones = self.obtener_repeticiones_tarea(
            tarea
        )
    ...
    def crear_intervalos_semanales(self, tarea):
        ...
        duracion_total = self.obtener_duracion_total_tarea(
            tarea
        )
    ...
    def formatear_mensaje_conflictos(
        ...
        for indice, conflicto in enumerate(conflictos, start=1):
            ...
            duracion_existente = self.obtener_duracion_repeticion(
                tarea_existente
            )
            repeticiones_existente = self.obtener_repeticiones_tarea(
                tarea_existente
            )
        ...
        for tarea in self.tareas:
            ...
            for inicio_candidata, fin_candidata in intervalos_candidata:
                ...
                for inicio_existente, fin_existente in intervalos_existente:
                    ...
                    inicio_canonico = (
                        inicio_solapamiento - desplazamiento
                    )
                    fin_canonico = (
                        fin_solapamiento - desplazamiento
                    )
                    ...
                    conflictos.append(
                        {
                            "tarea": tarea,
                            "inicio_candidata": (
                                inicio_candidata - desplazamiento
                            ),
                            "fin_candidata": (
                                fin_candidata - desplazamiento
                            ),
                            "inicio_existente": (
                                inicio_existente - desplazamiento
                            ),
                            "fin_existente": (
                                fin_existente - desplazamiento
                            ),
    ...
    def copiar_tarea(self, tarea_id):
        ...
        copia["repeticiones_totales"] = self.obtener_repeticiones_tarea(
            copia
        )
    ...
    def guardar_rutina_como_tarea(self):
        ...
        acciones_semanticas = json.loads(
            json.dumps(
                self.rutina_borrador_semantica
                or self.interpretar_acciones_semanticas(
                    acciones
                )
            )
        )
    ...
    def actualizar_motor_ejecucion(self):
        ...
        estado_repeticion = (
            self.calcular_estado_repeticiones(
                tarea,
                transcurrido,
            )
        )

        duracion_repeticion = estado_repeticion[
            "duracion_repeticion"
        ]

        repeticiones = estado_repeticion[
            "repeticiones"
        ]

        duracion_total = estado_repeticion[
            "duracion_total"
        ]
        ...
        transcurrido = estado_repeticion[
            "transcurrido_total"
        ]

        repeticion_actual = estado_repeticion[
            "repeticion_actual"
        ]

        transcurrido_repeticion = estado_repeticion[
            "transcurrido_repeticion"
        ]
        ...
        if acciones:
            ...
            if (
                ...
                if hasattr(self, "mensaje_visor"):
                    self.mensaje_visor.setText(
                        "BIN ESTÁ EJECUTANDO\n\n"
                        f"{tarea['nombre']}\n\n"
                        f"REPETICIÓN {repeticion_actual} "
                        f"DE {repeticiones}\n"
            ...
            self.actualizar_estado_cabecera_visor(
                ...
                (
                    f"· Repetición "
                    f"{repeticion_actual}/{repeticiones} "
                    f"· Acción {indice + 1}/{cantidad}"
                ),
                AMARILLO,
            )
    ...
    def pausar_ejecucion(self, tarea):
        ...
        if (
            acciones
            and duracion_total > 0
            and estado_repeticion["duracion_repeticion"] > 0
        ):
            cantidad = len(
                acciones
            )
    ...
    def continuar_ejecucion(self, tarea):
        ...
        if (
            acciones
            and tarea.get("accion_actual_indice")
            is not None
        ):
            detalle = (
                f"· Repetición "
                f"{repeticion_actual}/{repeticiones} "
                f"· Acción "
                f"{tarea['accion_actual_indice'] + 1}/"
                f"{len(acciones)}"
            )
        else:
            detalle = (
    ...
    def iniciar_grabadores_globales(
        ...
        if (
            pynput_mouse is None
            or pynput_keyboard is None
        ):
            QMessageBox.warning(
                self,
        ...
        def al_click(
            ...
            nombre_boton = (
                self.nombre_boton_mouse_global(
                    boton
                )
            )

            if presionado:
                self._mouse_botones_listener.add(
                    nombre_boton
                )
            else:
                self._mouse_botones_listener.discard(
                    nombre_boton
                )
        ...
        def al_presionar_tecla(
            ...
            if (
                ...
                and tecla
                == pynput_keyboard.Key.f12
            ):
                self.solicitud_detener_grabacion.emit()
                return

            info = (
                self.serializar_tecla_global(
                    tecla
                )
            )
        ...
        try:
            ...
            self.listener_teclado_global = (
                pynput_keyboard.Listener(
            ...
            QMessageBox.critical(
                ...
                "BIN no pudo iniciar la captura "
                "global de entrada.\n\n"
                f"{error}",
            )
    ...
    def interpretar_acciones_semanticas(
        ...
        def ids_acciones(
            ...
            for elemento in elementos:
                identificador = elemento.get(
                    "id"
                )

                if (
                    identificador is not None
                    and identificador not in ids
                ):
                    ids.append(
                        identificador
                    )
        ...
        def agregar_cambio_contexto(
            ...
            datos["proceso_antes"] = antes.get(
                "proceso"
            )
            datos["proceso_despues"] = despues.get(
                "proceso"
            )
        ...
        def crear_semantica(
            ...
            antes = contexto_inicial(
                elementos
            )

            despues = contexto_final(
                elementos
            )

            datos_semanticos = copia_segura(
                datos
                or {}
            )
        ...
        while indice < len(acciones):
            ...
            if (
                indice + 2 < len(acciones)
                and clave_atajo(accion)
                == ("ctrl", "l")
            ):
                accion_texto = acciones[
                    indice + 1
                ]

                accion_enter = acciones[
                    indice + 2
                ]

                if (
                    accion_texto.get("tipo")
                    == "escribir_texto"
                    and es_enter(
                        accion_enter
                    )
    ...
    def agrupar_acciones_grabadas(
        ...
        def copiar(
            ...
            id_original = copia.get(
                "id"
            )

            if (
                id_original is not None
                and id_original not in ids
            ):
                ids.append(
                    id_original
                )
                ...
                accion.update(
                    {
                        ...
                        "texto": contenido,
                        "descripcion": (
                            f'Escribir texto · "{contenido}"'
                        ),
                        "eventos_agrupados": len(
                            texto["ids"]
                        ),
                        "ids_origen": list(
                            texto["ids"]
        ...
        def clave_scroll(
            ...
            return (
                ...
                self.firma_contexto_agrupacion(
                    accion.get(
                        "contexto_objetivo"
                    )
                ),
        ...
        for original in originales:
            ...
            caracter = (
                self.extraer_caracter_agrupable(
                    accion
                )
            )
    ...
    def confirmar_click_global_pendiente(
        ...
        if accion_id is not None:
            QTimer.singleShot(
                ...
                self.completar_contexto_accion_global(
                    accion_id
                ),
            )
    ...
    def procesar_evento_mouse_global(
        ...
        if tipo_evento == "move":
            for boton in evento.get(
                ...
                estado = (
                    self.mouse_global_presionados.get(
                        boton
                    )
                )
        ...
        if tipo_evento == "up":
            ...
            estado = (
                self.mouse_global_presionados.pop(
                    boton,
                    None,
                )
            )
    ...
    def nombre_tecla_global(
        ...
        if (
            len(nombre) >= 2
            and nombre.startswith("f")
            and nombre[1:].isdigit()
        ):
            return nombre.upper()
    ...
    def procesar_evento_teclado_global(
        ...
        modificador = (
            self.modificador_teclado_global(
                nombre
            )
        )
        ...
        if tipo_evento == "down":
            if modificador:
                self.teclas_modificadoras_global.add(
                    modificador
                )

                return

            # Evita duplicar el mismo key-down si Windows
            # genera repetición mientras la tecla sigue abajo.
            if identificador in self.teclas_abajo_global:
                return

            self.teclas_abajo_global.add(
                identificador
            )
        ...
        if tipo_evento == "up":
            if modificador:
                self.teclas_modificadoras_global.discard(
                    modificador
                )

                return

            self.teclas_abajo_global.discard(
                identificador
            )

            datos_inicio = (
                self.acciones_teclado_en_curso.pop(
                    identificador,
                    None,
                )
            )
    ...
    def crear_barra_grabacion(
        ...
        self.barra_grabacion.setObjectName(
            "barra_grabacion"
        )
        ...
        self.label_barra_grabacion = QLabel(
            "● BIN grabando demostración"
        )

        self.boton_detener_barra = QPushButton(
        ...
        layout.addWidget(
            self.label_barra_grabacion
        )

        layout.addWidget(
            self.boton_detener_barra
        )
    ...
    def alternar_grabacion(
        ...
        if not self.grabando:
            ...
            iniciado = (
                self.iniciar_grabadores_globales()
            )
